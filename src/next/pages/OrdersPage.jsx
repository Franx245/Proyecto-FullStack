...
function isSessionExpiredQueryError(error) {
  const message = String(error?.message || "");
  return Number(error?.status || 0) === 401
    || String(error?.code || "") === "SESSION_EXPIRED"
    || /session expired/i.test(message);
}

function getQueryErrorMessage(error, fallbackMessage) {
  if (isSessionExpiredQueryError(error)) {
    return "Tu sesión venció. Volvé a ingresar para recuperar tus pedidos.";
  }

  return String(error?.message || fallbackMessage);
}

function sortOrdersByNewest(left, right) {
...
export default function OrdersPage() {
  ...
  const trackedOrdersQuery = useQuery({
    ...
    enabled: !isBootstrapping && isAuthenticated && trackedOrderIds.length > 0,
  });
  ...
  const paginatedOrders = useMemo(
    () => (isAuthenticated ? [...(myOrdersQuery.data?.orders ?? [])].sort(sortOrdersByNewest) : []),
    [isAuthenticated, myOrdersQuery.data?.orders]
  );
  const paginatedOrderIds = useMemo(
    () => new Set(paginatedOrders.map((order) => String(order.id))),
    [paginatedOrders]
  );
  const trackedOrders = useMemo(
    () => [...(trackedOrdersQuery.data?.orders ?? [])].sort(sortOrdersByNewest),
    [trackedOrdersQuery.data?.orders]
  );
  const recentTrackedOrders = useMemo(
    () => trackedOrders.filter((order) => !(isAuthenticated && page === 1 && paginatedOrderIds.has(String(order.id)))),
    [isAuthenticated, page, paginatedOrderIds, trackedOrders]
  );
  ...
  const trackedOrdersErrorMessage = trackedOrdersQuery.isError
    ...
    : "";
  const historyErrorMessage = myOrdersQuery.isError
    ? getQueryErrorMessage(myOrdersQuery.error, "No pudimos cargar tu historial completo.")
    : "";
  const hasVisibleOrders = recentTrackedOrders.length > 0 || paginatedOrders.length > 0;
  const shouldShowOrdersSkeleton = !hasVisibleOrders && (isBootstrapping || (isAuthenticated && (trackedOrdersQuery.isLoading || myOrdersQuery.isLoading)));
  const shouldShowHistorySync = isBootstrapping || (isAuthenticated && myOrdersQuery.isLoading);
  ...
  useEffect(() => {
    ...
    const watchedOrder = allKnownOrders.find((entry) => Number(entry.id) === Number(pendingPaymentFeedback.orderId));
    if (!watchedOrder) {
      return;
    }
  ...
  }, [allKnownOrders, pendingPaymentFeedback]);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-[900px] px-4 py-6">
        ...
        <div className="mb-4 space-y-3">
          {trackedOrdersNeedReauth ? (
            <div className="rounded-2xl border border-amber-400/25 bg-amber-400/10 px-4 py-3 text-sm text-amber-200">
              Tenés pedidos recientes guardados en este navegador, pero necesitás volver a iniciar sesión para recuperarlos.
            </div>
          ) : null}
          {trackedOrderIds.length > 0 && isAuthenticated && trackedOrdersQuery.isLoading ? (
          ...
          {shouldShowHistorySync ? (
            <div className="rounded-2xl border border-border bg-card/60 px-4 py-3 text-sm text-muted-foreground">
              {isBootstrapping ? "Validando tu sesión para cargar el historial completo..." : "Sincronizando tu historial completo..."}
            </div>
          ) : null}
          {trackedOrdersErrorMessage ? (
        ...
        {!isAuthenticated && trackedOrderIds.length === 0 && !isBootstrapping ? (
              ...
              <div key={index} className="rounded-2xl border border-border bg-card p-5">
                <div className="space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="h-6 w-24 animate-pulse rounded-full bg-secondary/80" />
                  </div>
                  <div className="h-28 animate-pulse rounded-2xl bg-background/60" />
                  <div className="space-y-2">
                    <div className="h-14 animate-pulse rounded-xl bg-background/60" />
                    <div className="h-14 animate-pulse rounded-xl bg-background/60" />
                  </div>
                </div>
              </div>
        ...
        ) : !hasVisibleOrders && (trackedOrdersErrorMessage || historyErrorMessage) ? (
          <div className="py-20 text-center text-muted-foreground">
            <ClipboardList className="mx-auto mb-4 h-14 w-14 opacity-20" />
            <p className="text-base font-semibold text-foreground">No pudimos cargar tus pedidos.</p>
            <p className="mt-2 text-sm">Probá de nuevo en unos segundos o volvé a iniciar sesión si la sesión venció.</p>
          </div>
        ) : trackedOrdersNeedReauth ? (
          <div className="py-20 text-center text-muted-foreground">
            <ClipboardList className="mx-auto mb-4 h-14 w-14 opacity-20" />
            <p className="text-base font-semibold text-foreground">Necesitás volver a ingresar para recuperar tus pedidos recientes.</p>
            <p className="mt-2 text-sm">Tus órdenes rastreadas siguen asociadas a esta cuenta, pero ahora requieren una sesión válida.</p>
            <Link href="/auth?redirect=/orders" className="mt-5 inline-flex h-11 items-center justify-center rounded-2xl bg-primary px-5 text-sm font-bold text-primary-foreground">
              Ingresar para ver pedidos
            </Link>
          </div>
        ) : !hasVisibleOrders ? (
          <div className="py-20 text-center text-muted-foreground">
          ...
          <div className="space-y-4">
            {recentTrackedOrders.length > 0 ? (
              <section className="space-y-4">
                <div className="rounded-2xl border border-border bg-card/60 px-4 py-3">
                  <p className="text-sm font-semibold text-foreground">Pedidos recientes rastreados</p>
                  <p className="mt-1 text-xs text-muted-foreground">Son las órdenes que este navegador viene siguiendo y que todavía no aparecen repetidas en la página actual del historial.</p>
                </div>
                {recentTrackedOrders.map((order, idx) => renderOrderCard(order, idx, "tracked"))}
              </section>
            ) : null}
            ...
            {paginatedOrders.length > 0 ? (
              <section className="space-y-4">
                <div className="rounded-2xl border border-border bg-card/60 px-4 py-3">
                  <p className="text-sm font-semibold text-foreground">Historial completo</p>
                  <p className="mt-1 text-xs text-muted-foreground">Página {page} de {totalPages} de tus pedidos asociados a la cuenta.</p>
                </div>
                {paginatedOrders.map((order, idx) => renderOrderCard(order, idx, `history-${page}`))}
              </section>
            ) : null}

            {shouldShowHistorySync && hasVisibleOrders ? (
              <div className="flex items-center gap-2 rounded-2xl border border-border bg-card/60 px-4 py-3 text-sm text-muted-foreground">
                <Loader2 className="h-4 w-4 animate-spin" />
